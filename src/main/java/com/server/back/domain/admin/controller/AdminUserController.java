...
public class AdminUserController {
    ...
    public ResponseEntity<ResultDto<List<AdminUserResDto>>> getNicknameUserList(@PathVariable String nickname){
    ...
    public ResponseEntity<ResultDto<List<AdminUserResDto>>> getAccountUserList(@PathVariable String account){
        List<AdminUserResDto> userList = adminUserService.getUserList("account", account);
        return ResponseEntity.ok(ResultDto.of(userList));
    }
    @GetMapping("/{account}")
    @ApiOperation(value = "회원 상세 정보를 반환합니다.", notes = "")
    public ResponseEntity<ResultDto<AdminUserInfoResDto>> getUserInfo(@PathVariable String account){
    ...
    public ResponseEntity<ResultDto<Boolean>> deleteUserInfo(@PathVariable String account){
